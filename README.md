# Curso de Posgrado: Sistemas de Bases de Datos Avanzados

**Descripción del Curso:** Este curso intensivo de tres semanas proporciona una introducción a nivel de posgrado a los principios y prácticas de los sistemas de bases de datos modernos. El curso cubrirá los fundamentos teóricos de la gestión de bases de datos, incluyendo modelos de datos, álgebra relacional y SQL, así como aspectos prácticos del diseño, implementación y administración de bases de datos. También se introducirán temas avanzados como bases de datos distribuidas, NoSQL y almacenamiento de datos.

**Instructor:** Servio Palacios, Ph.D.

**Contacto:** servio@mayanmind.ai

**Horario:** Jueves, Viernes y Sábados, 4 horas cada día.

**Ubicación:** Virtual

## Objetivos del Curso

Al finalizar este curso, los estudiantes serán capaces de:

* Comprender los conceptos fundamentales de los sistemas de bases de datos.
* Diseñar e implementar una base de datos relacional.
* Escribir consultas SQL complejas.
* Comprender los principios de la gestión de transacciones y el control de concurrencia.
* Familiarizarse con temas avanzados de bases de datos como bases de datos distribuidas y NoSQL.

## Libro de Texto Requerido

* "Database System Concepts" por Silberschatz, Korth y Sudarshan (7ma Edición)

## Evaluación

*   Análisis Crítico de Lecturas Científicas: 10%
*   Laboratorios: 20%
*   Proyecto Final - Capstone: 30%
*   Pruebas Cortas: 15%
*   Examen Final: 15%
*   Participación en Debates, Foros y Actividades: 10%

## Calendario del Curso

### Semana 1

*   **Jueves (4 horas):**
    *   **Tema:** Introducción y Organización del Curso, Evolución de los Sistemas de Base de Datos
    *   **Lecturas:** Silberschatz, Capítulos 1-4
*   **Viernes (4 horas):**
    *   **Tema:** El Modelo Relacional y Álgebra Relacional
    *   **Lectura de Artículo:** Presentación y discusión de un artículo científico.
    *   **Lecturas:** Silberschatz, Capítulos 5-6
*   **Sábado (4 horas):**
    *   **Tema:** SQL
    *   **Entrega:** Fecha límite para el Laboratorio 1.
    *   **Lecturas:** Silberschatz, Capítulos 7-8

### Semana 2

*   **Jueves (4 horas):**
    *   **Tema:** Diseño de Base de Datos
    *   **Lecturas:** Silberschatz, Capítulo 9
    *   **Quiz #1:** Relacionado al contenido de la semana 1 y lo que se hizo en el Miro Board.
*   **Viernes (4 horas):**
    *   **Tema:** Modelos NoSQL (Clave-Valor, Documentales, Columnares)
    *   **Lectura de Artículo:** Presentación y discusión de un artículo científico.
    *   **Lecturas:** Silberschatz, Capítulos 10-11
*   **Sábado (4 horas):**
    *   **Tema:** Bases de Datos Distribuidas y Replicación
    *   **Entrega:** Fecha límite para el Laboratorio 2.
    *   **Lecturas:** Silberschatz, Capítulos 14-15

### Semana 3

*   **Jueves (4 horas):**
    *   **Tema:** Integración y Análisis de Datos Empresariales, Bases de Datos de Grafos (TruenoDB)
    *   **Lecturas:** Silberschatz, Capítulos 19, 20 y 21
*   **Viernes (4 horas):**
    *   **Tema:** Seguridad, Privacidad y Gobierno, Arquitecturas Modernas y Tecnologías Emergentes
    *   **Lectura de Artículo:** Presentación y discusión de un artículo científico.
    *   **Lecturas:** Silberschatz, Capítulo 25
*   **Sábado (4 horas):**
    *   **Tema:** Presentaciones Finales del Proyecto Capstone
    *   **Entrega:** Proyecto Final - Capstone y Fecha límite para el Laboratorio 3.
